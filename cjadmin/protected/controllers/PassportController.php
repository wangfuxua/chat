...
class PassportController extends CommonController{
    ...
    public $loginflag = "__channel_admin_flag";
    public $model = '';
    public $uid;
    public function init() {
      ...
      //  $this->title = '买金网';
        ...
        if($this->uid && $m!="passport"){
            $this->redirect("/cjadmin/channel");
        }
    }
 ...
 * */
    ...
    function actionDoLogin(){
        ...
        //var_dump($_POST);die;
        $data['username'] = $username = Yii::app()->request->getParam('username');
        $data['pwd'] = $pwd = Yii::app()->request->getParam('pwd');
        $data['authcode'] = $authcode = Yii::app()->request->getParam('authcode');
        $data['autologin'] = $autologin = Yii::app()->request->getParam('autologin');
        if (!$data['username'] || !$data['pwd'] || !$data['authcode']) {
            $this->redirect('/cjadmin/passport/login/errormsg/必要信息输入不完整');
        }

        if (!$this->checkAuthcode($data['authcode'])){
            // 验证码错误
            $this->redirect('/cjadmin/passport/login/errormsg/验证码错误');
        }
        $r=$this->model->doLogin($username,$pwd);
        //  echo $r;die;
        if($r==-2){
            $this->redirect('/cjadmin/passport/login/errormsg/账号不存在');
        }
        if($r==0){
            $this->redirect('/cjadmin/passport/login/errormsg/密码错误');
        }
        $this->redirect('/cjadmin/channel/index');
    }
