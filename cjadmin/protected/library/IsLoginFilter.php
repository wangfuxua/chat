...
class IsLoginFilter extends CFilter {
	...
	public function filter($filterChain) {
		...
		$sql = "SELECT a.*, b.`role` FROM `admin_user` a LEFT JOIN `admin_role` b ON a.`role_id`=b.`id` WHERE a.`flag`='{$flag}'";
		// echo $sql; die;
			...
			if ('passport' != $ctrl) {
				...
				if (1 == $user['status']) {
					if ('center' != $ctrl) {
						...
						// 如果是律师，则需要补全所有资料
						if (!$user['name'] || !$user['email'] || !$user['address'] || !$usero['papersno'] || !$usero['company'] || !$usero['exp'] || !$usero['intro'] || !$usero['field'] || !$usero['org']) {
							Yii::app()->getController()->redirect('/center/user/msg/请补全个人资料');
						}
					}
